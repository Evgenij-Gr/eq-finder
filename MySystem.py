...
class TwoOscillators:
    ...
    # def Jac(self, fis):
    #     fi1, fi2 = fis
    #     return [[-np.cos(fi1) - self.paramK * np.cos(fi2 - fi1), self.paramK * np.cos(fi2 - fi1)],
    #             [self.paramK * np.cos(fi1 - fi2), -np.cos(fi2) - self.paramK * np.cos(fi1 - fi2)]]


# bounds = [(-0.1, 2*np.pi+0.1), (-0.01, 0.01), (-0.1, 2*np.pi+0.1), (-0.01, 0.01)]
# borders = [(-1e-15, +2 * np.pi + 1e-15), (-1e-6, 1e-6), (-1e-15, +2 * np.pi + 1e-15), (-1e-6, 1e-6)]
