...
def workerHeteroclinicNet(params, paramR, events, pset: sf.PrecisionSettings, proxs: sf.ProximitySettings, eqFinderParams):
    (i, a), (j, b) = params
    r = paramR
    ud = [0.5, a, b, r]
    ...
    result = fth.checkHeterocninicNet(osc, bordersEq, bounds, eqf, pset, proxs, 1000., events)
    return i, j, a, b, r, result

if __name__ == "__main__":
    if '-h' in sys.argv or '--help' in sys.argv:
        print("Usage: python HeteroclinicNet.py <pathToConfig> <outputMask> <outputDir>"
    ...
    start = time.time()
    ret = pool.map(partial(workerHeteroclinicNet, paramR = r,events = evtFlag, pset = ps, proxs = prox, eqFinderParams = eqFinderParams), itls.product(enumerate(alphas), enumerate(betas)))
    end = time.time()
    pool.close()
